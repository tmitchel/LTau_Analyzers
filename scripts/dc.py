    ...
    if zvar_name == None:
      ...
      for i, edge in enumerate(edges[1:]):
        ...
        if DCP_idx == None:
          ...
          zevents_dcp_plus = zevents[(zevents[dcp] > 0)]
          hists[i].fill(zevents_dcp_plus[xvar_name].values, zevents_dcp_plus[yvar_name].values, weight=zevents_dcp_plus['final_evtwt'].values) 

          zevents_dcp_minus = zevents[(zevents[dcp] <= 0)]
          hists[i+DCP_idx].fill(zevents_dcp_minus[xvar_name].values, zevents_dcp_minus[yvar_name].values, weight=zevents_dcp_minus['final_evtwt'].values) 

    data.drop(columns=['final_evtwt'], inplace=True)
...
def main(args):
    ...
    output_file = uproot.recreate('Output/templates/2D_htt_{}_{}_{}_fa3_{}_{}{}.root'.format(channel_prefix, ztt_name, syst_name, args.year, date, args.suffix))
    ...
    for syst, files in filelist.iteritems():
        ...
        for ifile in files:
            ...
            output_file['{}_0jet#{}'.format(channel_prefix, name)] = zero_jet_hist.to_numpy()
            ...
            output_file['{}_boosted#{}'.format(channel_prefix, name)] = boost_hist.to_numpy()
            ...
            output_file['{}_vbf#{}'.format(channel_prefix, name)] = vbf_hist.to_numpy()
            ...
            for i, cat in enumerate(vbf_cat_hists):
              output_file['{}_{}#{}'.format(channel_prefix, vbf_categories[i], name)] = cat.to_numpy()
            ...
            if args.syst and 'jetFakes' in name:
                for syst in boilerplate['fake_factor_systematics']:
                    ...
                    output_file['{}_0jet#jetFakes{}'.format(channel_prefix, jet_postfix)] = zero_jet_hist.to_numpy()
                    ...
                    output_file['{}_boosted#jetFakes{}'.format(channel_prefix, jet_postfix)] = boost_hist.to_numpy()
                    ...
                    output_file['{}_0vbf#jetFakes{}'.format(channel_prefix, jet_postfix)] = vbf_hist.to_numpy()
                    ...
                    for i, cat in enumerate(vbf_cat_hists):
                      output_file['{}_{}#jetFakes{}'.format(channel_prefix, vbf_categories[i], jet_postfix)] = cat.to_numpy()
