...
def build_histogram(name, bins, output_file, directory):
    ...
    return bh.Histogram(bh.axis.Regular(bins[0], bins[1], bins[2]))
...
def fill_histograms(config):
    ...
    config.hists.fill(xvar, weight=evtwt)
...
def main(args):
    ...
    output_file = uproot.recreate('Output/histograms/htt_{}_{}_{}_fa3_{}_{}{}.root'.format(channel_prefix, ztt_name, syst_name, args.year, args.date, args.suffix),)
        ...
        for itree in trees:
            ...
            for variable, bins in config_variables.iteritems():
                ...
                for obj in inclusive_hists.itervalues():
                    output_file['{}_inclusive#{}#{}'.format(channel_prefix, variable, obj['config'].name)] = obj['config'].queue.get().to_numpy()
                    # obj['config'].queue.get().Write()

                # output_file.cd('{}_0jet/{}'.format(channel_prefix, variable))
                for obj in zero_jet_hists.itervalues():
                    output_file['{}_0jet#{}#{}'.format(channel_prefix, variable, obj['config'].name)] = obj['config'].queue.get().to_numpy()
                    # obj['config'].queue.get().Write()

                # output_file.cd('{}_boosted/{}'.format(channel_prefix, variable))
                for obj in boosted_hists.itervalues():
                    output_file['{}_boosted#{}#{}'.format(channel_prefix, variable, obj['config'].name)] = obj['config'].queue.get().to_numpy()
                    # obj['config'].queue.get().Write()

                # output_file.cd('{}_vbf/{}'.format(channel_prefix, variable))
                for obj in vbf_hists.itervalues():
                    output_file['{}_vbf#{}#{}'.format(channel_prefix, variable, obj['config'].name)] = obj['config'].queue.get().to_numpy()
                    # obj['config'].queue.get().Write()
