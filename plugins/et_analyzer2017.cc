...
int main(int argc, char *argv[]) {
    ...
    // bveto weights
    bjet_weighter bveto_weights(2017, bveto_wp::medium);
    ...
    for (Int_t i = 0; i < nevts; i++) {
        ...
        } else if (jets.getNbtagMedium() < 1 && bjets.at(0).getBScore() < 0.4184) {
            histos->at("cutflow")->Fill(6., 1.);
        }  else {
            continue;
        }
        ...
        if (!isData && !isEmbed) {
            ...
            std::string single_eff_name = fabs(electron.getEta()) < 1.479 ? "e_trg27_trg32_trg35_embed_kit_ratio" : "e_trg27_trg32_trg35_kit_data";
            std::string el_leg_eff_name = fabs(electron.getEta()) < 1.479 ? "e_trg_EleTau_Ele24Leg_kit_ratio_embed" : "e_trg_EleTau_Ele24Leg_desy_data";
            std::string tau_leg_eff_name = fabs(electron.getEta()) < 1.479 ? "et_emb_LooseChargedIsoPFTau30_kit_ratio" : "et_LooseChargedIsoPFTau30_kit_data";
