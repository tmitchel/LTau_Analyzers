...
int main(int argc, char *argv[]) {
    ...
    for (Int_t i = 0; i < nevts; i++) {
        ...
        // tau isolation
        if (tau.getAgainstTightMuonDeep() && tau.getAgainstVVVLooseElectronDeep()) {
        ...
        bool found_bjets = (bjets.at(0).getBScore() >= 0.6321);

        // create regions
        bool signalRegion = (muon.getIso() < 0.15 && evt_charge == 0 && mt < 50 && !found_bjets);
        bool qcdRegion =    (muon.getIso() < 0.15 && evt_charge != 0 && mt < 50 && !found_bjets && muon.getIso() > 0.02);
        bool wjetsRegion =  (muon.getIso() < 0.15 && evt_charge == 0 && mt > 70 && !found_bjets);
        bool ttbarRegion =  (muon.getIso() < 0.15 && evt_charge == 0 && mt < 50 && found_bjets);

        bool isoRegion = (tau.getMediumIsoDeep() > 0);
        bool antiRegion = (tau.getMediumIsoDeep() == 0 && tau.getVVVLooseIsoDeep());

        // only keep the regions we need
        if (signalRegion || qcdRegion || wjetsRegion || ttbarRegion) {
        ...
        if (signalRegion) {
            ...
            tree_cat.push_back("wjetsRegion");
        } else if (ttbarRegion) {
            tree_cat.push_back("ttbarRegion");
        }

        if (isoRegion) {
            tree_cat.push_back("isoRegion");
        } else if (antiRegion) {
            tree_cat.push_back("antiRegion");
        }
